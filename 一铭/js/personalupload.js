...
$(function (){
		...
		$.ajax({
			...
			success:function (data){
				...
				if(window.sessionStorage.getItem('resubmitAppid')){
					// console.log("请求页面数据")
					updatas = '&update=1';
					var datan = 'appId='+window.sessionStorage.getItem('resubmitAppid')+addString;
					$.ajax({
	...
	$('#lsubmit').click(function (){
		...
		var laddAppid
		if(window.sessionStorage.getItem('resubmitAppid')){
			laddAppid = '&appId='+window.sessionStorage.getItem('resubmitAppid');
		}else {
			laddAppid = '';
		}
		...
		$.ajax({
			url: interface_uri + '/EmindStoreManager/appAction/addAppInfo?save=0',
			type: 'post',
			contentType: "application/x-www-form-urlencoded; charset=UTF-8",
			data: formDate,
			 beforeSend: function () {
