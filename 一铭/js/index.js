 ...
 **/
        ...
        $.ajax({
            ...
            beforeSend: function () {
                ...
                if ($("#lsjyx").val() == "") {
                    ...
                    // 终止请求发出
                    return false;
                }else if ($("#lqrmm").val() != $("#lmm").val()) {
                   alert('密码不一致')
                    return false;
                }else if(!$('#lyhxy')[0].checked){
                    alert('阅读并同意条款');
                    return false;[0]
                }
                // 忽略了其它的验证，自行完成
        ...
        $.ajax({
            ...
            beforeSend: function () {
                ...
                if ($("#lemail").val() == "") {
                    ...
                    // 终止请求发出
                    return false;
                }else if ($("#lemi-qrmm").val() != $("#lemil-mm").val()) {
                    alert("密码不一致");
                    //$("#lqrmm").val("密码不一致").fadeIn(500).delay(1500).fadeOut(500);
                    return false;
                }else if(!$('#lyhxy')[0].checked){
                    alert('阅读并同意条款');
                    return false;[0]
    ...
    //             sessionStorage.setItem("location", $("#province").val());
    ...
    $('#lemil-mm').bind('keyup',function (){
         $('#lemil-mm').css('color','#333230');
        if($('#lemil-mm').val()){
            var v = /^\w{6,18}$/g.test($('#lemil-mm').val());
            if(v){
